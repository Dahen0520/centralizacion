<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight text-center">
            {{ __('Editar Tipo de Organización') }}
        </h2>
    </x-slot>
    ...
    <div class="w-full max-w-6xl bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 border-t-8 border-blue-600 mx-auto mt-12">
        <div class="text-center mb-8">
            <p class="text-gray-600 dark:text-gray-400 text-base mb-4">Actualiza los datos del tipo de organización</p>
        ...
        <form method="POST" action="{{ route('tipo-organizacions.update', $tipoOrganizacion) }}">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="nombre" name="nombre" value="{{ old('nombre', $tipoOrganizacion->nombre) }}" required class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 dark:text-gray-200 dark:bg-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('nombre') border-red-500 @enderror">
                @error('nombre')
            ...
            <button type="submit" class="w-full py-3 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 transition duration-200 btn-hover-scale text-lg">
                Actualizar Tipo de Organización
            </button>
        </form>
    </div>
</x-app-layout>